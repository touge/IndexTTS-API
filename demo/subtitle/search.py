...
from opencc import OpenCC                      # 导入简繁转换工具
        ...
        for line in tqdm(target_lines, desc="Linearly Aligning Text"):  # 为每一行文本做对齐
            ...
            for i in range(search_start_idx, search_end_idx):
                for j in range(i, search_end_idx):
                    ...
                    # 在比较前，将 Whisper 输出的繁体（可能）文本统一转换为简体
                    simplified_sub_sequence = self.cc.convert(normalized_sub_sequence)

                    current_score = fuzz.token_set_ratio(normalized_line, simplified_sub_sequence)  # 在统一字符集下计算模糊匹配得分
            ...
            match_threshold = 75     # 恢复原始匹配阈值，依靠简繁转换保证精度
